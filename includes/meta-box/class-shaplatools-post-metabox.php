		...
		public function meta_box_callback() {
			...
			if ( $value ) {
				...
				foreach ( $thumbs as $thumb ) {
					$output .= '<li class="shaplatools_gallery_list_item">' . wp_get_attachment_image( $thumb,
							array( 75, 75 ) ) . '</li>';
				}
			}
			...
			$html .= sprintf( '<input type="hidden" value="%1$s" id="shaplatools_images_ids" name="_shaplatools_images_ids">',
				$value );
			$html .= sprintf( '<a href="#" id="shaplatools_gallery_btn" class="shaplatools_gallery_btn">%s</a>',
				$btn_text );
