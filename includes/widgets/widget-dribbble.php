...
add_action( 'widgets_init', create_function( '', 'return register_widget( "Shapla_Dribbble_Widget" );' ) );
...
class Shapla_Dribbble_Widget extends WP_Widget{
	...
	public function __construct() {
		parent::__construct(
			'shapla-dribbble', // Base ID
			__( 'Shapla Dribbble Shots', 'shapla' ), // Name
			array( 'description' => __( 'Display your latest Dribbble shots.', 'shapla' ), ) // Args
		);
	}
...
?>
	...
	<div class='shapla-dribbble-widget'>
		...
		<ul class="dribbbles">
			...
			<?php endforeach; ?>
			<?php else: ?>
			<?php _e( 'Please check your dribbble username', 'shapla' ); ?>
			<?php endif; ?>
		</ul>
	</div>
...
<?php
	...
	function form( $instance ){
		$defaults = array(
			'title' => __( 'Dribbble Shots', 'shapla' ),
			'dribbble_name' => 'codeshapla',
			'dribbble_shots' => 4,
			'new_window' => ''
		);
		...
		<p>
			<label for="<?php echo $this->get_field_id('dribbble_name'); ?>"><?php _e( 'Username:', 'shapla' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('dribbble_shots'); ?>"><?php _e( 'Number of shots to show:', 'shapla' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('new_window'); ?>"><?php _e( 'Open links in new window?', 'shapla' ); ?></label>
		</p>
