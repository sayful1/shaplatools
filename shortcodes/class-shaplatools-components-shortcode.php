...
if ( ! class_exists( 'Shaplatools_Components_Shortcode' ) ):
	...
	class Shaplatools_Components_Shortcode {
		...
		public function shapla_map( $atts ) {
			...
			if ( '' != $api_key ) {
				wp_enqueue_script( 'google-maps',
					add_query_arg( 'key', $api_key, 'https://maps.googleapis.com/maps/api/js' ), array( 'jquery' ) );
			} else {
				if ( current_user_can( 'edit_posts' ) ) :
					echo '<p class="shapla-alert shapla-red">';

					/* translators: %s is a link, do not remove/modify it. */
					echo sprintf( esc_html__( 'To be able to use Google Maps, you first need to set an %s.',
						'shaplatools' ),
						sprintf( '<a href="' . admin_url( 'options-general.php?page=shaplatools' ) . '">%1$s</a>',
							esc_html__( 'API key', 'shaplatools' ) )
					);
					echo '</p>';
				endif;
		...
		function shapla_tabs( $atts, $content = null ) {
			...
			if ( count( $tab_titles ) ) {
				$output .= '<section id="shapla-tabs-' . rand( 1,
						100 ) . '" class="shapla-section shapla-tabs shapla-tabs--' . esc_attr( $args['style'] ) . '"><div class="shapla-tab-inner">';
