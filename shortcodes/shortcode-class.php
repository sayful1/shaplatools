...
class Shapla_Shortcodes {
	...
	function __construct( $popup ) {
		if ( file_exists( dirname( __FILE__ ) . '/config.php' ) ) {
			...
			$this->append_error( __( 'Config file does not exist', 'shapla' ) );
		}
	}
		...
		if ( isset( $shapla_shortcodes ) && is_array( $shapla_shortcodes ) ) {
			...
			foreach ( $this->params as $pkey => $param ) {
				...
				switch ( $param['type'] ) {
					...
					case 'image';
						$output = $row_start;
						$output .= '<a href="#" data-type="image" data-text="Insert Image" class="shapla-open-media button" title="' . esc_attr__( 'Choose Image', 'shapla' ) . '">' . __( 'Choose Image', 'shapla' ) . '</a>';
					...
					case 'video';
						$output = $row_start;
						$output .= '<a href="#" data-type="video" data-text="Insert Video" class="shapla-open-media button" title="' . esc_attr__( 'Choose Video', 'shapla' ) . '">' . __( 'Choose Video', 'shapla' ) . '</a>';
						$output .= '<input class="shapla-input" type="text" name="' . $pkey . '" id="' . $pkey . '" value="' . $param['std'] . '" />';
						$output .= $row_end;
						$this->append_output( $output );
					break;
