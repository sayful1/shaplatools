...
class ShaplaTools_Widget extends WP_Widget {
	...
	/**
	 * ShaplaTools_Widget constructor.
	 */
	...
	function get_cached_widget( $args ) {
		$cache = wp_cache_get( $this->widget_id, 'shaplatools-widget' );
	...
	public function cache_widget( $args, $content ) {
		...
		wp_cache_set( $this->widget_id, $cache, 'shaplatools-widget' );
	}
	...
	public function flush_widget_cache() {
		wp_cache_delete( $this->widget_id, 'shaplatools-widget' );
	}
			...
			switch ( $setting['type'] ) {
				...
				case 'text' :
                    ...
                    <p>
                        ...
                        <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"
                               ...
                               value="<?php echo esc_attr( $value ); ?>" <?php if ( isset( $setting['placeholder'] ) ) {
				...
				case 'checkbox' :
                    ...
                    <p>
                        <label for="<?php echo $this->get_field_id( $key ); ?>">
                            <input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"
                                   name="<?php echo $this->get_field_name( $key ); ?>" type="text"
                                   value="1" <?php checked( 1, esc_attr( $value ) ); ?>/>
				...
				case 'categories':
                    ...
                    <p>
                        ...
                        <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"
                                name="<?php echo $this->get_field_name( $key ); ?>">
                            <option value="-1"><?php _e( 'All', 'shaplatools' ); ?></option>
				...
				case 'textarea' :
                    ...
                    <p>
                        ...
                        <textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"
                                  name="<?php echo $this->get_field_name( $key ); ?>"
                                  rows="<?php echo $setting['rows']; ?>"><?php echo esc_html( $value ); ?></textarea>
                    </p>
					<?php
					break;
			}
