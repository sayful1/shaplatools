...
class MetaBox {
	...
	public function portfolio_image() {
		...
		add_meta_box( 'postimagediv', __( 'Portfolio Featured Image', 'shaplatools' ), 'post_thumbnail_meta_box',
			'portfolio', 'side', 'low' );
	}
		...
		if ( 'skill' == $column_name ) {
			...
			if ( ! $terms = get_the_terms( get_the_ID(), $column_name ) ) {
				...
				foreach ( $terms as $term ) {
					$termlist[] = '<a href="' . esc_url( add_query_arg( $column_name, $term->slug,
							admin_url( 'edit.php?post_type=portfolio' ) ) ) . ' ">' . ucfirst( $term->name ) . '</a>';
				}
