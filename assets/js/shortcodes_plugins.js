...
function base64_encode(h) {
    var d = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
    var c, b, a, m, l, k, j, n, g = 0, o = 0, f = "", e = [];
    if (!h) {
        return h
    }
...
jQuery(document).ready(function ($) {
    var shaplas = {
        ...
        cLoadVals: function () {
            ...
            // adds the filled-in shortcode as hidden input
            $('#_shapla_cshortcodes').remove();
            $('.child-clone-rows').prepend('<div id="_shapla_cshortcodes" class="hidden">' + shortcodes + '</div>');

            // add to parent shortcode
            this.loadVals();
            pShortcode = $('#_shapla_ushortcode').text().replace('{{child_shortcode}}', shortcodes);
        ...
        media: function () {
            ...
            if (insertButton.data('type') === "image") {
                frame_title = ShaplaShortcodes.media_frame_image_title;
            } else if (insertButton.data('type') === "video") {
                frame_title = ShaplaShortcodes.media_frame_video_title;
            }
            ...
            insertButton.on('click', function (e) {
                ...
                shapla_media_frame = wp.media.frames.shapla_media_frame = wp.media({
                    className: 'media-frame shapla-media-frame',
                    frame: 'select',
                    multiple: false,
                    title: frame_title,
                    library: {
        ...
        load: function () {
            var shaplas = this,
                tbWindow = $('#TB_window'),
                popup = $('#shapla-popup'),
                form = $('#shapla-sc-form', popup),
                shortcode = $('#_shapla_shortcode', form).text(),
                popupType = $('#_shapla_popup', form).text(),
                uShortcode = '',
