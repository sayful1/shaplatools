...
class Shapla_Instagram_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( false !== $instagram ) :
            ...
            <a class="instagram-follow-link"
               href="https://instagram.com/<?php echo esc_html( $username ); ?>"><?php printf( __( 'Follow %1$s on Instagram',
					'shaplatools' ), esc_html( $username ) ); ?></a>

		<?php elseif ( ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) && ( defined( 'WP_DEBUG_DISPLAY' ) && false !== WP_DEBUG_DISPLAY ) ) : ?>
            <div id="message" class="error"><p><?php _e( 'Error: We were unable to fetch your instagram feed.',
						'shaplatools' ); ?></p></div>
		<?php endif;
	...
	function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'Instagram Username:',
					'shaplatools' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Photo Count:',
					'shaplatools' ); ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'size' ); ?>"><?php _e( 'Photo Size:',
					'shaplatools' ); ?></label>
            <select id="<?php echo $this->get_field_id( 'size' ); ?>"
                    name="<?php echo $this->get_field_name( 'size' ); ?>" class="widefat">
                <option value="thumbnail" <?php selected( 'thumbnail', $instance['size'] ) ?>><?php _e( 'Thumbnail',
						'shaplatools' ); ?></option>
                <option value="low_resolution" <?php selected( 'low_resolution',
					$instance['size'] ) ?>><?php _e( 'Low Resolution', 'shaplatools' ); ?></option>
                <option value="standard_resolution" <?php selected( 'standard_resolution',
					$instance['size'] ) ?>><?php _e( 'High Resolution', 'shaplatools' ); ?></option>
            </select>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id( 'cachetime' ); ?>"><?php _e( 'Cache time (in hours):',
					'shaplatools' ); ?></label>
