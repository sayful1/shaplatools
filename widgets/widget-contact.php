...
class ShaplaContactFormAJAX extends WP_Widget {
	...
	function widget( $args, $instance ) {
        ...
        <form id="theForm">
            ...
            <textarea name="message" id="message" cols="30" rows="5"
                      placeholder="<?php _e( 'Message', 'shaplatools' ); ?>"></textarea>

            <input name="action" type="hidden" value="the_ajax_hook"/>
            <!-- this puts the action the_ajax_hook into the serialized form -->
            <input id="submit_button" value="<?php _e( 'Send Message', 'shaplatools' ); ?>" type="button"
                   onClick="submit_me();"/>
	...
	function shapla_contact_form_widget_function( $instance ) {
		...
		if ( isset( $_POST['shapla_contact_widget_nonce'] ) && wp_verify_nonce( $_POST['shapla_contact_widget_nonce'],
				'shapla_contact_widget_action' ) ) {
			...
			if ( isset( $_SESSION['timeout'] ) && ( $_SESSION['timeout'] + 10 * 60 ) > time() ) {
				...
				if ( $accept_time > time() ) {
					// session timed out
					$error .= '<span class="error">' . __( 'Your cannot resend message within ',
							'shaplatools' ) . $diff_time . __( ' minute(s).', 'shaplatools' ) . '</span>';
				}
			} else {

				// Validate fullname with PHP
				if ( strlen( $fullname ) < 3 ) {
					$error    .= '<span class="error">' . __( 'Your name should be at least 3 characters.',
							'shaplatools' ) . '</span>';
					$hasError = true;
				}
				...
				if ( ! is_email( $email ) ) {
					$error    .= '<span class="error">' . __( 'You entered an invalid email address.',
							'shaplatools' ) . '</span>';
					$hasError = true;
				}
				...
				if ( strlen( $message ) < 15 ) {
					$error    .= '<span class="error">' . __( 'Your message should be at least 15 characters.',
							'shaplatools' ) . '</span>';
					$hasError = true;
				}
			...
			if ( isset( $emailSent ) && $emailSent == true ) {
				echo '<div class="shapla-alert shapla-alert--green">' . __( 'Thanks, your email was sent successfully.',
						'shaplatools' ) . '</div>';
	...
	function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php _e( 'Contact Form Email Address:',
					'shaplatools' ) ?></label>
            <input type="email" class="widefat" id="<?php echo $this->get_field_id( 'email' ); ?>"
                   name="<?php echo $this->get_field_name( 'email' ); ?>" value="<?php if ( isset( $email ) ) {
				echo esc_attr( $email );
			} ?>"/>
            <small><?php _e( 'Enter the email address where you would like to receive emails from the contact form.',
					'shaplatools' ); ?></small>
        </p>
		<?php
	}
