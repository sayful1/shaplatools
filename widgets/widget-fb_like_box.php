	...
	function form( $instance ) {
        ...
        <p>
            ...
            <input type="text" class="widefat" id="<?php echo $this->get_field_id( 'app_id' ); ?>"
                   name="<?php echo $this->get_field_name( 'app_id' ); ?>" value="<?php if ( isset( $app_id ) ) {
				echo esc_attr( $app_id );
			} ?>"/>

            <small><?php _e( 'Dont\'t know your App ID? Head on over to <a target="_blank" href="https://developers.facebook.com/">FB Developer</a> and create App ID. Still need help? visit <a target="_blank" href="http://sayful1.wordpress.com/2014/06/12/how-to-get-facebook-api-key/">here</a>.',
					'shaplatools' ) ?></small>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id( 'href' ); ?>"><?php _e( 'Facebook Page URL',
					'shaplatools' ) ?></label>
            <input type="text" class="widefat" id="<?php echo $this->get_field_id( 'href' ); ?>"
                   name="<?php echo $this->get_field_name( 'href' ); ?>" value="<?php if ( isset( $href ) ) {
				echo esc_attr( $href );
			} ?>"/>
            <small><?php _e( 'The absolute URL of the Facebook Page that will be liked. e.g. https://www.facebook.com/FacebookDevelopers',
					'shaplatools' ) ?></small>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id( 'width' ); ?>"><?php _e( 'Width', 'shaplatools' ) ?></label>
            <input type="number" class="widefat" id="<?php echo $this->get_field_id( 'width' ); ?>"
                   name="<?php echo $this->get_field_name( 'width' ); ?>" value="<?php if ( isset( $width ) ) {
				echo esc_attr( $width );
			} ?>" min="180" max="500">
            <small><?php _e( 'The width of the plugin in pixels. Minimum is 180. Maximum is 500.',
					'shaplatools' ) ?></small>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php _e( 'Height', 'shaplatools' ) ?></label>
            <input type="number" class="widefat" id="<?php echo $this->get_field_id( 'height' ); ?>"
                   name="<?php echo $this->get_field_name( 'height' ); ?>" value="<?php if ( isset( $height ) ) {
				echo esc_attr( $height );
			} ?>" min="70">
            <small><?php _e( 'The height of the plugin in pixels. The default height varies based on number of faces to display, and whether the stream is displayed.',
					'shaplatools' ) ?></small>
        </p>
        <p>
            <input type="checkbox" class="widefat" id="<?php echo $this->get_field_id( 'showfaces' ); ?>"
                   name="<?php echo $this->get_field_name( 'showfaces' ); ?>"
                   value="1" <?php echo( $showfaces == "true" ? "checked='checked'" : "" ); ?> />
            <label for="<?php echo $this->get_field_id( 'showfaces' ); ?>"><?php _e( 'Show Friends\' Faces',
					'shaplatools' ) ?></label>
        </p>
        <p>
            <input type="checkbox" class="widefat" id="<?php echo $this->get_field_id( 'stream' ); ?>"
                   name="<?php echo $this->get_field_name( 'stream' ); ?>"
                   value="1" <?php echo( $stream == "true" ? "checked='checked'" : "" ); ?> />
            <label for="<?php echo $this->get_field_id( 'stream' ); ?>"><?php _e( 'Show Page Posts',
					'shaplatools' ) ?></label>
        </p>
        <p>
            <input type="checkbox" class="widefat" id="<?php echo $this->get_field_id( 'small_header' ); ?>"
                   name="<?php echo $this->get_field_name( 'small_header' ); ?>"
                   value="1" <?php echo( $small_header == "true" ? "checked='checked'" : "" ); ?> />
            <label for="<?php echo $this->get_field_id( 'small_header' ); ?>"><?php _e( 'Use Small Header',
					'shaplatools' ) ?></label>
        </p>
        <p>
            <input type="checkbox" class="widefat" id="<?php echo $this->get_field_id( 'hide_cover' ); ?>"
                   name="<?php echo $this->get_field_name( 'hide_cover' ); ?>"
                   value="1" <?php echo( $hide_cover == "true" ? "checked='checked'" : "" ); ?> />
            <label for="<?php echo $this->get_field_id( 'hide_cover' ); ?>"><?php _e( 'Hide Cover Photo',
					'shaplatools' ) ?></label>
        </p>
		<?php
	}
